...
#full service name
SERVICE_NAME="blog-server-1.0-SNAPSHOT"
JAR_NAME=$SERVICE_NAME\.jar
PID=$SERVICE_NAME\.pid
CONFIG_DIR=${BASE_PATH}"/conf/"
JAVA_MEM_OPTS=" -server -Xmx400m -Xms400m -Xmn100m -Dfile.encoding=UTF-8 -Dsun.jnu.encoding=UTF-8 -XX:PermSize=256m -XX:+DisableExplicitGC
...
-XX:+UseCMSInitiatingOccupancyOnly -XX:CMSInitiatingOccupancyFraction=70 -XX:+HeapDumpOnOutOfMemoryError -XX:HeapDumpPath=/tmp/blog-server-SNAPSHOT.dump -Djava.net.preferIPv4Stack=true"
DEBUG=""
LOG_FILE=blog-server.log
...
function startService(){
        ...
        if [ -n "$PIDS" ]; then
            ...
            nohup java $JAVA_OPTS -jar $JAR_NAME --spring.config.location=/home/admin/blog-server-1.0-SNAPSHOT/conf/ >$SERVICE_DIR/logs/$LOG_FILE 2>&1 &
